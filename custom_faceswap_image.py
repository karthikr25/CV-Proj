# -*- coding: utf-8 -*-
"""
Created on Mon Sep 27 11:37:54 2021
...
for source_face in source_faces: 
    source_face_landmark = frontal_face_predictor(source_image_grayscale,source_face)
    source_face_landmark_points = []
    
    for landmark_no in range(0,68):
        x_point = source_face_landmark.part(landmark_no).x
        y_point = source_face_landmark.part(landmark_no).y
        source_face_landmark_points.append((x_point,y_point))
        
        cv2.circle(source_image,(x_point,y_point),2,(255,0,0),-1)
        cv2.putText(source_image,str(landmark_no),(x_point,y_point),cv2.FONT_HERSHEY_SIMPLEX,0.2,(255,255,255))
    ...
    #cv2.imshow("2:Convexhull source image face",source_image)
    
    cv2.fillConvexPoly(source_image_canvas, source_face_convexhull, 255)
    #cv2.imshow("3: Draw convex hull polygon over canvas ",source_image_canvas)
    ...
    for triangle in triangles_array:
        ...
        #cv2.imshow("5: Draw all Delaunay triangles in source image", source_face_image)
...
for destination_face in destination_faces: 
    destination_face_landmark = frontal_face_predictor(destination_image_grayscale,destination_face)
    destination_face_landmark_points = []
    
    for landmark_no in range(0,68):
        x_point = destination_face_landmark.part(landmark_no).x
        y_point = destination_face_landmark.part(landmark_no).y
        destination_face_landmark_points.append((x_point,y_point))
        
        cv2.circle(destination_image,(x_point,y_point),2,(255,0,0),-1)
        cv2.putText(destination_image,str(landmark_no),(x_point,y_point),cv2.FONT_HERSHEY_SIMPLEX,0.2,(255,255,255))
        #cv2.imshow("1:Landmark points of destination", destination_image)
    
    #print(destination_face_landmark_points)    
    ...
    cv2.polylines(destination_image, [destination_face_convexhull], True, (255,0,0),3)
    #cv2.imshow("2:Convexhull destination image face",destination_image)
...
for i, triangle_index_points in enumerate(source_triangle_index_points_list):
    ...
    _, mask_created_triangle = cv2.threshold(new_dest_face_canvas_area_gray, 200, 255, cv2.THRESH_BINARY_INV)
